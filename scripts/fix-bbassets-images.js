...
async function main() {
  console.log("Fetching products with broken bbassets.com URLs...");
  ...
  // Group by product name to avoid duplicate searches
  ...
  for (const productName of uniqueProducts) {
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('product-images').upload(fileName, buffer, { contentType: res.headers.get('content-type') || 'image/jpeg' });
  ...
  console.log(`Done! Fixed ${updated} out of ${uniqueProducts.length} unique products.`);
}
